...
Git 的版本回退速度非常快，因为 Git 在内部有个指向当前分支（此处是 master）的 HEAD 指针，`refs/heads/master` 文件里保存当前 master 分支的最新 `commit id`。当我们在回退版本的时候，Git仅仅是给 `refs/heads/master` 中存储一个特定的 version，可以简单理解成如下示意图：
